...
function createRandomEncounter() {
    ...
    let selectedEncounter = selectRandomArrayEntry(potentialEncounters);
    function populateCurrentEncounter () {
        CURRENT_ENCOUNTER.title = selectedEncounter.title ?? "Title Not Found";
        CURRENT_ENCOUNTER.description = selectedEncounter.description ?? "Description Not Found";
        CURRENT_ENCOUNTER.requiredEnemy = selectedEncounter.requiredEnemy ?? "No Required Enemy Found";
        CURRENT_ENCOUNTER.enemyType = selectedEncounter.enemyType ?? "Required Enemy Type Not Found";
        CURRENT_ENCOUNTER.singleEnemy = selectedEncounter.singleEnemy ?? "Single Enemy Required Not Found";
        CURRENT_ENCOUNTER.requiredEnemy = selectedEncounter.requiredEnemy ?? null;
        CURRENT_ENCOUNTER.minCR = selectedEncounter.minCR ?? "No Min CR";
        CURRENT_ENCOUNTER.maxCR = selectedEncounter.maxCR ?? "No Max CR";
        CURRENT_ENCOUNTER.rewardTable = selectedEncounter.rewardTable ?? selectedEncounter.reward ?? "No Reward Table Found";
        CURRENT_ENCOUNTER.dmNotes = selectedEncounter.dmNotes ?? "No DM Notes Found";    
        CURRENT_ENCOUNTER.potentialEnemies();  
    }
    selectedEncounter == undefined ? console.error("No encounter found.") : populateCurrentEncounter();
    console.timeEnd();
}
